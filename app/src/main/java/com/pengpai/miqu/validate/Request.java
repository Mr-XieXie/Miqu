...
import com.pengpai.miqu.base.utils.RSAUtils;
...
public class Request {
    ...
    /*//获取短信验证码
    public void sendSmsCode(Bundle values, RequestCallback callback) {

        SendSmsCodeParam param=new SendSmsCodeParam();
        String privateKey=values.getString("appkey");
        param.appId=values.getString("appid");
        String phoneNum=values.getString("phoneNum");
        try {
            param.mobileNumber= AESUtils.EncodeAES(phoneNum, privateKey);
        } catch (Exception e) {
            e.printStackTrace();
        }
        param.userIp=values.getString("userIp");
        param.accessToken=values.getString("accessToken");
        param.sign=param.generateSign(privateKey);

        doCommonRequest(Constant.sendSmsCode_url, param,callback);
    }

    //短信验证码校验
    public void smsCodeVerifi(Bundle values, RequestCallback callback) {

        SmsCodeVerifiParam param=new SmsCodeVerifiParam();
        String privateKey=values.getString("appkey");
        param.appId=values.getString("appid");
        String phoneNum=values.getString("phoneNum");
        try {
        ...
        param.userIp=values.getString("userIp");
        param.smsCode=values.getString("smsCode");
        param.sign=param.generateSign(privateKey);

        doCommonRequest(Constant.smsCodeVerifi_url, param,callback);
    }*/
...
//            private String expandparams;//expandparams	扩展参数	2	Map	map(key,value)
        ...
        TokenValidateParameter.BodyBean bodyBean = new TokenValidateParameter.BodyBean();
        TokenValidateParameter.HeaderBean headerBean = new TokenValidateParameter.HeaderBean();
        headerBean.setStrictcheck("0");
        headerBean.setVersion("1.0");
        headerBean.setMsgid(generateNonce32());
        headerBean.setSystemtime(getCurrentTime());
        headerBean.setAppid(Constant.APP_ID);
        headerBean.setApptype("5");
        bodyBean.setToken(values.getString(Constant.KEY_TOKEN));
        headerBean.setSign(RSAUtils.generalSign(Constant.APP_ID + values.getString(Constant.KEY_TOKEN), Constant.privatekey));
//        headerBean.setSign(TokenValidateParameter.hamcSha1(Constant.APP_ID+values.getString(Constant.KEY_TOKEN),"B50BBEF6C4CD4FA8"));
        params.setBody(bodyBean);
        params.setHeader(headerBean);
    ...
    //对外请求接口 本机号码校验
    public  void doCommonRequest(final String url, RequestParameter params,  final RequestCallback callback) {
        Log.e(TAG, "request https url : " + url + ">>>>>>> PARAMS : " + params.toJson().toString());
        new HttpUtils().requestHttp(url, params.toJson().toString(),  new HttpUtils.Response() {
            @Override
            public void onSuccess(String result) {
